...
letter : src/c/letter.c
	$(avr) -mmcu=$(chip) -DF_CPU=$(freq) -Os -c -o build/letter.elf src/c/letter.c

word : src/c/word.c letter
...
test_letter : src/test_letter.c SPI_led word interrupt
	$(avr) -mmcu=$(chip) -DF_CPU=$(freq) -Os -o build/main.elf src/test_letter.c build/word.elf build/letter.elf build/SPI_led.elf build/interrupt.elf
	$(obj) -O binary build/main.elf build/main.bin
